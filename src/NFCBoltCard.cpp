...
static void nfc_task_code(void *pvParams)
{
    ...
    while (s_boltcardRunning)
    {
        ...
        {
            // Ring-Login: NTAG 424 DNA in SUN mode carries a plain HTTPS URL
            // matching the tagid scan path. Only handle in Authy/Identity mode.
            if (authyConfig.enabled) {
                String sunUrl = String((char *)fileBuf);
                int scanIdx = sunUrl.indexOf("/api/v1/scan/");
                if (scanIdx >= 0) {
                    int extStart = scanIdx + 13; // skip "/api/v1/scan/"
                    int qPos = sunUrl.indexOf('?', extStart);
                    if (qPos > extStart) {
                        String extId = sunUrl.substring(extStart, qPos);
                        // Parse p and c from query string
                        String query = sunUrl.substring(qPos + 1);
                        String pVal, cVal;
                        int pos = 0;

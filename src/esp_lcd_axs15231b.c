 ...
 *
 * Stripped-down for ZapBox: panel-only, no touch.
 * Adapted from JC3248W535EN reference (esp_lcd_axs15231b).
 */
                                       ...
                                       esp_lcd_panel_handle_t *ret_panel)
{
    ...
    axs15231b->base.disp_off = panel_axs15231b_disp_off;
    *ret_panel = &(axs15231b->base);

    ESP_LOGI(TAG, "axs15231b panel created");
    return ESP_OK;
...
static esp_err_t panel_axs15231b_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(axs15231b, io, LCD_CMD_COLMOD, (uint8_t[]){axs15231b->colmod_val}, 1), TAG, "send command failed");

    const axs15231b_lcd_init_cmd_t *init_cmds = axs15231b->init_cmds ? axs15231b->init_cmds : vendor_specific_init_default;
    uint16_t init_cmds_size = axs15231b->init_cmds ? axs15231b->init_cmds_size :
                              (sizeof(vendor_specific_init_default) / sizeof(axs15231b_lcd_init_cmd_t));

    for (int i = 0; i < init_cmds_size; i++) {
        if (init_cmds[i].cmd == LCD_CMD_MADCTL) {
            axs15231b->madctl_val = ((uint8_t *)init_cmds[i].data)[0];
        } else if (init_cmds[i].cmd == LCD_CMD_COLMOD) {
...
static esp_err_t panel_axs15231b_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    size_t len = (size_t)(x_end - x_start) * (y_end - y_start) * axs15231b->fb_bits_per_pixel / 8;
    if (y_start == 0) {
...
{
    axs15231b_panel_t *axs15231b = __containerof(panel, axs15231b_panel_t, base);
    if (mirror_x) axs15231b->madctl_val |= LCD_CMD_MX_BIT;
    else axs15231b->madctl_val &= ~LCD_CMD_MX_BIT;
    if (mirror_y) axs15231b->madctl_val |= LCD_CMD_MY_BIT;
    else axs15231b->madctl_val &= ~LCD_CMD_MY_BIT;
...
{
    axs15231b_panel_t *axs15231b = __containerof(panel, axs15231b_panel_t, base);
    if (swap_axes) axs15231b->madctl_val |= LCD_CMD_MV_BIT;
    else axs15231b->madctl_val &= ~LCD_CMD_MV_BIT;
    return tx_param(axs15231b, axs15231b->io, LCD_CMD_MADCTL, (uint8_t[]){axs15231b->madctl_val}, 1);
}
...
{
    axs15231b_panel_t *axs15231b = __containerof(panel, axs15231b_panel_t, base);
    return tx_param(axs15231b, axs15231b->io, off ? LCD_CMD_DISPOFF : LCD_CMD_DISPON, NULL, 0);
}

...
// also channel CH06, so the gauge only runs while CH06 is unconfigured; every
...
void initBattery();        // configure the ADC; no-op when CH06 is in use
void batteryLoop();        // call from loop(); samples internally every 10 s
bool batteryAvailable();   // false when CH06 is configured (pin is an output)
bool batteryCharging();    // true while USB powers the board (no valid cell reading)
...
inline bool batteryCharging()  { return false; }
